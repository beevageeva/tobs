...
Creamos una lista(fichero listCenter-M37) con las imagenes  que hay que alinear(las tipo "object" del centro del cúmulo (con el numero del nombre  de 96 a 105)) y otra a cual añadimos 'a' antes de la extensión (fichero listCenterAligned-M37) con alignImages.py (ejecutando align desde pyraf, sale un error, porque?)
...
\end{verbatim}
\item 
Hacemos una media de las columnas 4(SKYSIGMA) y 5(FWHM) con
...
\end{verbatim}
Esto va a crear el fichero M37New/object/R/Nov30103a.fits-daofind con las magnitudes de todas las estrellas encontradas en la imagen

\item Después ejecutamos phot para obtener los ficheros con las magnitudes
...
Cogemos una imagen de referencia (object/V/Nov30098a.fits) establecemos los valores de datapars.fwhmpsf y sigma a los valores correspondientes a esta imagen (como antes: el valor medio obtenido despues de seleccionar un par de estrellas con daoedit en la imagen correspondiente)
...
\item allstar para una imagen en iraf cl
...
\item ejecutar allstar para todas las estrellas del centro del cúmulo usando la misma imagen psf
\begin{verbatim}
...
ejecutamos imalign para alinear todas las imagenes segun la imagen de referencia para cada objeto pg
\begin{verbatim}
ecl>imalign.interp_type = "nearest" 
